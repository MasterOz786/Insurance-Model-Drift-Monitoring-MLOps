...
def _generate_synthetic_insurance_data(n_samples=100):
    ...
    data = {
        ...
        'Gender': np.random.choice(['Male', 'Female', ''], n_samples, p=[0.5, 0.45, 0.05]),
        ...
        'VehicleAge': np.random.choice(['< 1 Year', '1-2 Year', '> 2 Year', ''], n_samples, p=[0.4, 0.3, 0.25, 0.05]),
        'PastAccident': np.random.choice(['Yes', 'No', ''], n_samples, p=[0.3, 0.6, 0.1]),
        'AnnualPremium': [f"£{np.random.uniform(100, 5000):,.2f} " for _ in range(n_samples)],
