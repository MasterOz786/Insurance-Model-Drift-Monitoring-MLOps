...
def validate_data_quality(**context) -> bool:
    ...
    try:
        ...
        is_stock_data = all(col in sample_df.columns for col in ['open', 'high', 'low', 'close', 'volume'])
        is_insurance_data = all(col in sample_df.columns for col in ['AnnualPremium', 'Age', 'RegionID'])
        ...
        if is_stock_data:
            ...
            required_columns = ['AnnualPremium', 'Age', 'RegionID', 'Gender', 'PastAccident', 'HasDrivingLicense']
        else:
        ...
        min_rows = 50 if is_stock_data else 50  # Minimum threshold for insurance data (allows for small batches)
        if len(df) < min_rows:
        ...
        numeric_check_col = 'close' if is_stock_data else ('RegionID' if is_insurance_data else key_columns[0] if key_columns else None)
        ...
        if is_stock_data and df.index.dtype == 'datetime64[ns]':
            ...
            if days_old > 7:
                ...
                logger.info(f"✓ Data freshness: Latest data is {days_old} days old (PASS)")
